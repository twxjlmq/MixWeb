	...
	styleUI : {
		...
		Bootstrap : {
			...
			modal : {
				...
				icon_resizable : "glyphicon glyphicon-import"
			},
			celledit : {
			...
			inlinedit : {
				inputClass : 'form-control',
				icon_edit_nav : "fa-pencil-square-o",
				icon_add_nav : "fa-plus",
				icon_save_nav : "fa-floppy-o",
			...
			navigator : {
				...
				icon_newbutton_nav : "glyphicon glyphicon-new-window"
			},
			grouping : {
...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		var p = $.extend(true,{
			 ...
			 * 修改人：lmq
			 * 修改原因：
			 * 1.默认语言改为：中文；默认样式改为：bootstrap；
			 * 2.添加Bootstrap面板（Panel）主题样式--属性；
			 * 3.添加表格样式--属性
			 */
			regional :  "cn",
			styleUI : "Bootstrap",
		 ...
		 * 修改时间：2016年１月12日
		 * 修改人：lmq
		 * 修改原因：支持Bootstrap的panel样式（既：支持－－带表格的面版）
		 */
		gv = $("<div "+getstyle(stylemodule, 'viewBox', false, 'ui-jqgrid-view panel '+ts.p.theme)+" role='grid'></div>");
		/***************************************/
		...
		for(i=0;i<this.p.colNames.length;i++){
			 ...
			 * 修改内容：添加序号标识
			 */
			thead += "<th id='"+ts.p.id+"_"+ts.p.colModel[i].name+"' role='columnheader' "+getstyle(stylemodule,'headerBox',false, "ui-th-column ui-th-"
					+dir+(ts.p.colModel[i].name=='rn'?' jqgrid-header-rownum':''))+" "+ tooltip+">";
			/*************************************/
		 ...
		 * 修改时间:2016年１月１２日；
		 * 修改人：lmq
		 * 修改内容：给表格设置表格样式（如：bootstrap的表格样式）
		 */
		if(ts.p.tableStyle == null) {
			$(this).addClass(getstyle(stylemodule,"rowTable", true, 'ui-jqgrid-btable ui-common-table')).append(firstr);
		} else {
			$(this).addClass(ts.p.tableStyle+" ui-jqgrid-btable ui-common-table").append(firstr);
		}
		/*******************************/
		...
		var hTable = null;
		if(ts.p.tableStyle == null) {
			hTable = $("<table "+getstyle(stylemodule,'headerTable',false,'ui-jqgrid-htable ui-common-table')+" style='width:"+ts.p.tblwidth+"px' role='presentation' aria-labelledby='gbox_"+this.id+"'></table>").append(thead);
		} else {
			hTable = $("<table class=\""+ts.p.tableStyle+" ui-jqgrid-htable ui-common-table\" style='width:"+ts.p.tblwidth+"px' role='presentation' aria-labelledby='gbox_"+this.id+"'></table>").append(thead);
		}
		/********************************/
		...
		if(ts.p.pager){
			 ...
			 * 修改人：lmq
			 * 修改内容：
			 * 如果表格有分页，则在.panel中添加元素中添加.panel-footer；
			 * 并把分页元素添加到.panel-footer元素下
			 */
			$(gv).append("<div class='panel-footer'></div>");
			var $panelFooter = $(gv).find(".panel-footer");
			$(ts.p.pager).addClass(getstyle(stylemodule,'pagerBox', true,'ui-jqgrid-pager')).appendTo($panelFooter);
			/*****************************************/
		 ...
		 * 修改时间：2016年01月13日
		 * 修改人：lmq
		 * 修改内容：表格添加Bootstrap标题样式(panel-heading)
		 */
		var titleIconHtml = "";
		if(ts.p.tableTitleIcon != null && '' != ts.p.tableTitleIcon) {
			titleIconHtml = "<span class=\"table-title-icon "+iconbase+" "+ts.p.tableTitleIcon+"\"></span> ";
		}
		$(grid.cDiv).append(arf).append(titleIconHtml+"<span class='ui-jqgrid-title'>"+ts.p.caption+"</span>")
		.addClass("panel-heading ui-jqgrid-titlebar ui-jqgrid-caption"+(dir==="rtl" ? "-rtl" :"" )+" "+getstyle(stylemodule,'gridtitleBox',true));
...
$.jgrid.extend({
	...
	navGrid : function (elem, p, pEdit, pAdd, pDel, pSearch, pView) {
		...
		return this.each(function() {
			...
			for(i = 0; i<clone; i++) {
				...
				navtbl = $("<div class=\"btn-group btn-group-sm nav-btn-group\" role=\"group\"></div>"),
				...
				if (o.add) {
					...
					tbd = $("<button type=\"button\" class=\"btn "+o.addStyle+"\"></button>");
					$(tbd).append("<span class='"+commonstyle.icon_base +" " +o.addicon+"'></span> "+o.addtext+"");
					$(navtbl).append(tbd);
					$(tbd,navtbl)
				...
				if (o.edit) {
					...
					//$("tr",navtbl).append(tbd);
					tbd = $("<button type=\"button\" class=\"btn "+o.editStyle+"\"></button>");
					$(tbd).append("<span class='"+commonstyle.icon_base +" " +o.editicon+"'></span> "+o.edittext+"");
					$(navtbl).append(tbd);
					$(tbd,navtbl)
				...
				if (o.view) {
					...
					//$("tr",navtbl).append(tbd);
					tbd = $("<button type=\"button\" class=\"btn "+o.viewStyle+"\"></button>");
					$(tbd).append("<span class='"+commonstyle.icon_base +" " +o.viewicon+"'></span> "+o.viewtext+"");
					$(navtbl).append(tbd);
					$(tbd,navtbl)
				...
				if (o.del) {
					...
					//$("tr",navtbl).append(tbd);
					tbd = $("<button type=\"button\" class=\"btn "+o.delStyle+"\"></button>");
					$(tbd).append("<span class='"+commonstyle.icon_base +" " +o.delicon+"'></span> "+o.deltext+"");
					$(navtbl).append(tbd);
					$(tbd,navtbl)
				...
				if (o.search) {
					...
					//$("tr",navtbl).append(tbd);
					tbd = $("<button type=\"button\" class=\"btn "+o.searchStyle+"\"></button>");
					$(tbd).append("<span class='"+commonstyle.icon_base +" " +o.searchicon+"'></span> "+o.searchtext+"");
					$(navtbl).append(tbd);
					$(tbd,navtbl)
				...
				if (o.refresh) {
					...
					//$("tr",navtbl).append(tbd);
					tbd = $("<button type=\"button\" class=\"btn "+o.refreshStyle+"\"></button>");
					$(tbd).append("<span class='"+commonstyle.icon_base +" " +o.refreshicon+"'></span> "+o.refreshtext+"");
					$(navtbl).append(tbd);
					$(tbd,navtbl)
	...
	navButtonAdd : function (elem, p) {
		...
		return this.each(function() {
			...
			var findnav = $(".nav-btn-group",elem)[0], $t = this,
			//getstyle = $.jgrid.getMethod("getStyleUI"),
			...
			if (findnav) {
				...
				//var tbd = $("<td></td>");
				var button = $("<button type=\"button\" class=\"btn "+p.style+"\"></button>");
				...
				if(p.buttonicon.toString().toUpperCase() === "NONE") {
                    $(button).append(p.caption);
				} else	{
					$(button).append("<span class='"+iconbase+" "+p.buttonicon+"'></span> "+p.caption+"");
				}
				if(p.id) {$(button).attr("id",p.id);}
				if(p.position==='first'){
					if($(findnav).find("button").length === 0) {
						$(findnav).append(button);
					} else {
						$("button:eq(0)",findnav).before(button);
					}
